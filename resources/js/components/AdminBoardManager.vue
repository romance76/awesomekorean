...
<div>
  ...
  <div class="mb-4 flex items-start justify-between">
    <div>
      <div class="text-xs text-ink-muted">관리자 › 게시판 관리 › {{ label }}</div>
      ...
      <p class="text-xs text-ink-faint mt-0.5">게시글·카테고리·설정·포인트·배너·신고를 한 페이지에서 관리합니다</p>
    </div>
  </div>
  ...
  <div v-if="majorTypes && majorTypes.length" class="flex gap-2 mb-4 flex-wrap">
    <button @click="setMajorType('')"
      ...
      :class="!activeMajorType ? 'bg-amber-400 text-white border-amber-400' : 'bg-white text-ink-light border-gray-200 hover:bg-amber-50'">
      전체 ({{ overview.total || 0 }})
    </button>
    <button v-for="mt in majorTypes" :key="mt.key" @click="setMajorType(mt.key)"
      class="px-4 py-2 rounded-xl text-sm font-bold border transition-colors"
      :class="activeMajorType === mt.key ? 'bg-amber-400 text-white border-amber-400' : 'bg-white text-ink-light border-gray-200 hover:bg-amber-50'">
      {{ mt.label }} ({{ mt.count }})
    </button>
  </div>
  ...
  <div class="bg-white rounded-t-2xl border border-b-0 border-gray-100">
    <div class="flex overflow-x-auto">
      <button v-for="t in tabs" :key="t.key" @click="activeTab=t.key"
        ...
        <AppIcon v-if="t.icon" :name="t.icon" :size="14" />{{ t.label }}
        <span v-if="t.badge" class="ml-0.5 bg-red-500 text-white text-[11px] font-bold px-1.5 rounded-full">{{ t.badge }}</span>
      </button>
    </div>
  </div>
  ...
  <div class="bg-white rounded-b-2xl border border-t-0 border-gray-100 p-4 min-h-[400px]">
    ...
    <div v-else-if="activeTab==='cat'">
      <div class="flex justify-between items-center mb-3">
        <div class="text-sm text-ink-light">
          ...
          <span v-else class="badge-blue !text-[11px] ml-1">설정 저장</span>
        </div>
        <div class="flex gap-2">
          <button @click="addCategory" class="bg-blue-600 hover:bg-blue-700 text-white rounded-lg px-3 py-1.5 text-sm transition-colors">+ 추가</button>
      ...
      <div v-if="categories.length === 0" class="text-center text-ink-muted py-8 text-sm">카테고리가 없습니다. "+ 추가" 버튼을 눌러주세요.</div>
      <div v-else class="space-y-2">
        <div v-for="(c, i) in categories" :key="i" class="flex items-center gap-2 border border-gray-100 rounded-xl p-2 bg-gray-50">
          <span class="text-ink-faint cursor-move">☰</span>
          <input v-model="c.icon" placeholder="🏷" class="input-soft px-2 py-1 w-14 text-center bg-white" />
          <input v-model="c.name" placeholder="이름" class="input-soft px-2 py-1 flex-1 bg-white" />
          <input v-model="c.slug" placeholder="slug" class="input-soft px-2 py-1 w-28 bg-white" />
          ...
          <button @click="removeCategory(i)" class="text-red-500 text-xs px-2 hover:text-red-600 transition-colors">삭제</button>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="activeTab==='set'">
      <div class="text-sm text-ink-light mb-3">이 게시판만의 기본 설정입니다. 키 형식: <code class="bg-gray-100 px-1 rounded">board.{{ slug }}.*</code></div>
      <div class="space-y-3">
        <div v-for="(def, key) in settingSchema" :key="key" class="flex items-center gap-3 border-b border-gray-50 pb-2">
          <label class="text-sm flex-1">
            ...
            <div class="text-[11px] text-ink-faint">{{ key }}</div>
          </label>
          <template v-if="def.type==='bool'">
            <input type="checkbox" v-model="settingValues[key]" class="w-4 h-4" />
          </template>
          <template v-else-if="def.type==='select'">
            <select v-model="settingValues[key]" class="input-soft w-auto px-2 py-1">
              <option v-for="o in def.options" :key="o.value" :value="o.value">{{ o.label }}</option>
            </select>
          </template>
          <template v-else-if="def.type==='number'">
            <input type="number" v-model.number="settingValues[key]" class="input-soft px-2 py-1 w-28" />
          </template>
          <template v-else>
            <input type="text" v-model="settingValues[key]" class="input-soft px-2 py-1 w-56" />
          </template>
        </div>
      </div>
      <div class="mt-4 text-right">
        <button @click="saveSettings" class="btn-primary px-4 py-2">설정 저장</button>
      </div>
    </div>

    <!-- 💰 포인트 -->
    <div v-else-if="activeTab==='point'">
      <div class="text-sm text-ink-light mb-3">💡 <strong>{{ label }} 전용</strong> 포인트 규칙. 전역 포인트는 [시스템 › 포인트 설정]에서 관리합니다.</div>
      <table class="w-full text-sm border border-gray-100">
        <thead class="bg-gray-50 text-ink-light">
          <tr>
        ...
        <tbody>
          <tr v-for="(def, key) in pointSchema" :key="key" class="border-t border-gray-50" :class="def.is_deduction ? 'bg-red-50' : ''">
            <td class="p-2">
              <div class="font-medium text-ink">{{ def.label }}</div>
              <div class="text-[11px] text-ink-faint">{{ key }}</div>
            </td>
            <td class="p-2"><input type="number" v-model.number="pointValues[key]" class="input-soft px-2 py-1" /></td>
            <td class="p-2"><input type="number" v-model.number="pointValues[key + '_daily_max']" class="input-soft px-2 py-1" placeholder="-" /></td>
          </tr>
        </tbody>
      </table>
      <div class="mt-4 text-right">
        <button @click="savePoints" class="btn-primary px-4 py-2">포인트 규칙 저장</button>
      </div>
    </div>
    ...
    <div v-else-if="activeTab==='ban'">
      ...
      <div v-if="bannerStats" class="grid grid-cols-5 gap-2 mb-3">
        <div class="bg-gray-50 rounded-xl p-2 text-center"><div class="text-xs text-ink-muted">전체</div><div class="font-bold text-ink">{{ bannerStats.total }}</div></div>
        <div class="bg-green-50 rounded-xl p-2 text-center"><div class="text-xs text-ink-muted">활성</div><div class="font-bold text-green-700">{{ bannerStats.active }}</div></div>
        <div class="bg-yellow-50 rounded-xl p-2 text-center"><div class="text-xs text-ink-muted">대기</div><div class="font-bold text-yellow-700">{{ bannerStats.pending }}</div></div>
        <div class="bg-blue-50 rounded-xl p-2 text-center"><div class="text-xs text-ink-muted">노출</div><div class="font-bold text-blue-700">{{ bannerStats.total_impressions }}</div></div>
        <div class="bg-purple-50 rounded-xl p-2 text-center"><div class="text-xs text-ink-muted">매출(P)</div><div class="font-bold text-purple-700">{{ bannerStats.total_revenue }}</div></div>
      </div>
      <div v-if="banners.length === 0" class="text-center text-ink-muted py-8 text-sm">이 게시판에 등록된 광고가 없습니다.</div>
      <table v-else class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="b in banners" :key="b.id" class="border-t border-gray-50 hover:bg-amber-50/40 transition-colors">
            <td class="p-2 font-medium text-ink">{{ b.title }}</td>
            ...
            <td class="p-2 text-[11px] text-ink-muted">{{ b.start_date }} ~ {{ b.end_date }}</td>
            <td class="p-2 text-center text-xs">{{ b.impressions }}/{{ b.clicks }}</td>
            <td class="p-2 text-center">{{ b.total_cost }}</td>
            <td class="p-2 text-center">
              <span class="text-[11px] px-2 py-0.5 rounded-full font-semibold" :class="statusClass(b.status)">{{ b.status }}</span>
            </td>
            <td class="p-2 text-center">
    ...
    <div v-else-if="activeTab==='rep'">
      <div class="text-sm text-ink-light mb-3">이 게시판에 대한 신고 대기 목록</div>
      <div v-if="reports.length === 0" class="text-center text-ink-muted py-8 text-sm">신고가 없습니다.</div>
      <table v-else class="w-full text-sm">
        <thead class="bg-gray-50 text-ink-light">
          <tr>
        ...
        <tbody>
          <tr v-for="r in reports" :key="r.id" class="border-t border-gray-50">
            ...
            <td class="p-2 text-center"><span class="badge-gray !text-[11px]">{{ r.status }}</span></td>
            <td class="p-2 text-[11px] text-ink-faint">{{ (r.created_at||'').slice(0,10) }}</td>
            <td class="p-2 text-center">
              <button @click="updateReport(r, 'resolved')" class="text-xs text-green-600 mr-1 hover:underline">해결</button>
              <button @click="updateReport(r, 'rejected')" class="text-xs text-ink-muted hover:underline">기각</button>
            </td>
          </tr>
        </tbody>
      </table>
...
const tabs = computed(() => {
  ...
  const rest = [
    ...
    { key: 'point', icon: 'coins',        label: '포인트' },
    { key: 'ban',   icon: 'megaphone',    label: '배너/광고', badge: bannerStats.value?.pending || 0 },
    { key: 'rep',   icon: 'alert-circle', label: '신고', badge: overview.value?.pending_reports || 0 },
  ]
