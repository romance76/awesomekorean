...
// 수동 좌석 배치 (좌측=오른쪽정렬, 우측=왼쪽정렬 → 테이블쪽으로)
const seatPositions = [
  { x: 50, y: 80 },   // 0: 나
  { x: 24, y: 76 },   // 1: 좌하 — 오른쪽정렬
  { x: 10, y: 54 },   // 2: 좌 — 오른쪽으로 이동
  { x: 12, y: 30 },   // 3: 좌상 — 오른쪽으로 이동
  { x: 32, y: 17 },   // 4: 상좌
  { x: 68, y: 17 },   // 5: 상우
  { x: 86, y: 30 },   // 6: 우상 — 왼쪽으로 이동
  { x: 88, y: 54 },   // 7: 우 — 왼쪽으로 이동
  { x: 76, y: 76 },   // 8: 우하 — 왼쪽정렬
]

// 딜러 칩: 각 좌석에서 테이블 안쪽(중앙 방향)으로 충분히 떨어진 위치
const dealerChipOffsets = [
  { dx: 0,  dy: -14 },  // 0: 나 — 카드 위 중앙
  { dx: 10, dy: -8 },   // 1: 좌하 → 우상 방향
  { dx: 12, dy: 0 },    // 2: 좌 → 오른쪽
  { dx: 10, dy: 10 },   // 3: 좌상 → 우하 방향
  { dx: 5,  dy: 12 },   // 4: 상좌 → 아래
  { dx: -5, dy: 12 },   // 5: 상우 → 아래
  { dx: -10,dy: 10 },   // 6: 우상 → 좌하 방향
  { dx: -12,dy: 0 },    // 7: 우 → 왼쪽
  { dx: -10,dy: -8 },   // 8: 우하 → 좌상 방향
]
