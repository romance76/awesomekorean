  ...
  <div class="bg-white rounded-2xl shadow-xl w-full max-w-5xl max-h-[92vh] overflow-hidden flex flex-col">
    <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between bg-amber-50">
      <div class="flex items-center gap-2">
        <span class="icon-chip w-7 h-7 bg-white text-amber-600"><AppIcon name="user" :size="15" /></span>
        <span class="font-bold text-ink">회원 상세 정보</span>
        <span v-if="data?.user" class="text-sm text-ink-light">— {{ data.user.name }} ({{ data.user.email }})</span>
      </div>
      <button @click="$emit('close')" class="text-ink-muted hover:text-ink transition-colors"><AppIcon name="x" :size="20" /></button>
    </div>
    ...
    <div v-else-if="data" class="flex-1 overflow-hidden flex flex-col">
      ...
      <div class="px-5 pt-3 grid grid-cols-5 gap-2">
        ...
        <div class="bg-purple-50 rounded-xl p-2 text-center">
          <div class="text-xs text-ink-muted">전체 게시</div>
        ...
        <div class="bg-red-50 rounded-xl p-2 text-center">
          <div class="text-xs text-ink-muted">상태</div>
      ...
      <div class="px-5 pt-3 flex gap-1 border-b border-gray-100 overflow-x-auto">
        <button v-for="t in tabs" :key="t.key" @click="tab=t.key"
          ...
          <span v-if="count(t.key) > 0" class="ml-0.5 bg-gray-100 text-ink-light text-[11px] px-1 rounded">{{ count(t.key) }}</span>
        </button>
      </div>
      ...
      <div class="flex-1 overflow-y-auto px-5 py-3">
        ...
        <div v-if="tab==='info'">
          <div class="grid grid-cols-2 gap-3">
            <div><label class="input-label !text-xs !mb-0.5">이름</label><input v-model="data.user.name" class="input-soft px-3 py-1.5" /></div>
            <div><label class="input-label !text-xs !mb-0.5">닉네임</label><input v-model="data.user.nickname" class="input-soft px-3 py-1.5" /></div>
            <div><label class="input-label !text-xs !mb-0.5">이메일</label><input v-model="data.user.email" class="input-soft px-3 py-1.5" /></div>
            <div><label class="input-label !text-xs !mb-0.5">전화</label><input v-model="data.user.phone" class="input-soft px-3 py-1.5" /></div>
            <div><label class="input-label !text-xs !mb-0.5">도시</label><input v-model="data.user.city" class="input-soft px-3 py-1.5" /></div>
          ...
          <div class="mt-3 text-xs text-ink-faint">가입: {{ data.user.created_at?.slice(0,10) }} · 로그인 {{ data.user.login_count||0 }}회 · 최근 {{ data.user.last_login_at?.slice(0,10)||'-' }}</div>
          <button @click="saveUser" class="btn-primary mt-3 px-5 py-2">저장</button>
        </div>

        <!-- 결제 -->
        <div v-if="tab==='payments'">
          <div v-if="!data.payments?.length" class="py-6 text-center text-ink-muted text-sm">결제 내역 없음</div>
          <table v-else class="w-full text-xs">
            <thead class="bg-gray-50 text-ink-light"><tr><th class="p-2 text-left">주문#</th><th class="p-2">날짜</th><th class="p-2">금액</th><th class="p-2">포인트</th><th class="p-2">상태</th><th class="p-2">Stripe</th></tr></thead>
            <tbody>
              <tr v-for="p in data.payments" :key="p.id" class="border-t border-gray-50">
                ...
                <td class="p-2"><span class="px-1.5 py-0.5 rounded-full text-[11px] font-semibold" :class="statusClass(p.status)">{{ p.status }}</span></td>
                <td class="p-2 text-[11px] text-ink-faint truncate max-w-[120px]">{{ p.stripe_payment_id }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <!-- 포인트 -->
        <div v-if="tab==='points'">
          <div v-if="!data.points?.length" class="py-6 text-center text-ink-muted text-sm">포인트 내역 없음</div>
          <table v-else class="w-full text-xs">
            ...
            <tbody>
              <tr v-for="pt in data.points" :key="pt.id" class="border-t border-gray-50">
                <td class="p-2 text-ink-muted">{{ pt.created_at?.slice(0,16).replace('T',' ') }}</td>
                <td class="p-2">{{ pt.reason }}</td>
                <td class="p-2 text-[11px] text-ink-muted">{{ pt.type }}</td>
                <td class="p-2 text-right font-bold" :class="pt.amount>0?'text-green-600':'text-red-600'">{{ pt.amount>0?'+':'' }}{{ pt.amount }}P</td>
                <td class="p-2 text-right text-ink-light">{{ pt.balance_after }}P</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div v-if="tab==='comments'">
          <div v-if="!data.comments?.length" class="py-6 text-center text-ink-muted text-sm">댓글 없음</div>
          <div v-for="c in data.comments" :key="c.id" class="py-2 border-b border-gray-50 text-xs">
            <div class="text-ink-light">{{ c.content }}</div>
            <div class="text-[11px] text-ink-faint mt-0.5">{{ c.created_at?.slice(0,10) }} · 좋아요 {{ c.like_count||0 }}</div>
          </div>
        </div>
        <!-- 장터 -->
        ...
        <div v-if="tab==='banners'">
          <div v-if="!data.banners?.length" class="py-6 text-center text-ink-muted text-sm">광고 신청 내역 없음</div>
          <table v-else class="w-full text-xs">
            <thead class="bg-gray-50 text-ink-light"><tr><th class="p-2 text-left">제목</th><th class="p-2">페이지</th><th class="p-2">위치</th><th class="p-2">기간</th><th class="p-2">비용</th><th class="p-2">상태</th></tr></thead>
            <tbody>
              <tr v-for="b in data.banners" :key="b.id" class="border-t border-gray-50">
                <td class="p-2">{{ b.title }}</td>
                <td class="p-2 text-[11px]">{{ b.page }}</td>
                <td class="p-2 text-[11px] text-center">{{ b.position }}</td>
                <td class="p-2 text-[11px] text-ink-muted">{{ b.start_date }} ~ {{ b.end_date }}</td>
                <td class="p-2 text-right">{{ b.total_cost }}P</td>
                <td class="p-2 text-center"><span class="text-[11px] px-1.5 py-0.5 rounded-full font-semibold" :class="statusClass(b.status)">{{ b.status }}</span></td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div v-if="tab==='reports'">
          <div v-if="!data.reports_filed?.length" class="py-6 text-center text-ink-muted text-sm">신고 내역 없음</div>
          <div v-for="r in data.reports_filed" :key="r.id" class="py-2 border-b border-gray-50 text-xs">
            <div class="flex justify-between">
              <span class="font-medium text-ink">{{ r.reportable_type?.split('\\').pop() }} #{{ r.reportable_id }}</span>
              <span class="text-[11px] px-1.5 py-0.5 rounded-full font-semibold" :class="statusClass(r.status)">{{ r.status }}</span>
            </div>
            <div class="text-red-600">{{ r.reason }}</div>
            <div class="text-[11px] text-ink-faint">{{ r.created_at?.slice(0,10) }}</div>
          </div>
        </div>
      </div>
    </div>
...
const tabs = [
  ...
  { key:'jobs',       icon:'briefcase',      label:'구인' },
  { key:'realestate', icon:'building',       label:'부동산' },
  { key:'events',     icon:'calendar',       label:'이벤트' },
  { key:'clubs',      icon:'users',          label:'동호회' },
  { key:'qa',         icon:'help-circle',    label:'Q&A' },
  { key:'banners',    icon:'megaphone',      label:'광고' },
