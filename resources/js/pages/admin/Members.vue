  ...
  <div class="bg-white rounded-xl shadow-sm border p-3 mb-4">
    <div class="flex flex-wrap gap-2">
      <select v-model="roleFilter" @change="loadUsers()" class="border rounded-lg px-2 py-1.5 text-xs outline-none">
        <option value="">전체 회원</option><option value="admin">관리자</option><option value="user">일반</option><option value="super_admin">슈퍼관리자</option>
      </select>
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="u in users" :key="u.id"
              ...
              <td class="px-3 py-2.5 text-center"><span class="text-[10px] px-2 py-0.5 rounded-full font-bold" :class="u.role==='admin'||u.role==='super_admin'?'bg-purple-100 text-purple-700':'bg-gray-100 text-gray-600'">{{ {super_admin:'슈퍼',admin:'관리자',user:'일반'}[u.role] || u.role }}</span></td>
    ...
    <div v-if="activeUser" class="w-3/5">
      <div class="bg-white rounded-xl shadow-sm border overflow-hidden sticky top-4">
        ...
        <div v-else-if="userData" class="max-h-[70vh] overflow-y-auto">
          ...
          <div v-show="userTab==='info'" class="p-4">
            <div class="grid grid-cols-2 gap-3">
              ...
              <div><label class="text-[10px] text-gray-500">역할</label>
                <select v-model="userData.user.role" class="w-full border rounded px-2 py-1.5 text-sm mt-0.5"><option value="user">일반</option><option value="admin">관리자</option><option value="super_admin">슈퍼</option></select></div>
          ...
          <!-- 관리 -->
          <div v-show="userTab==='manage'" class="p-4 space-y-3">
            <div class="flex gap-2">
              <button v-if="!userData.user.is_banned" @click="banCurrentUser" class="bg-red-100 text-red-700 px-4 py-2 rounded-lg text-sm font-bold hover:bg-red-200">🚫 정지</button>
              <button v-else @click="unbanCurrentUser" class="bg-green-100 text-green-700 px-4 py-2 rounded-lg text-sm font-bold hover:bg-green-200">✅ 정지 해제</button>
            </div>
            <div v-if="userData.user.is_banned" class="text-sm text-red-500">사유: {{ userData.user.ban_reason || '없음' }}</div>
            <div>
