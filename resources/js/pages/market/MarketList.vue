  ...
  <div class="max-w-7xl mx-auto px-4 py-5">
    ...
    <div class="lg:hidden mb-3">
      <div class="flex items-center justify-between mb-2">
        <h1 class="flex items-center gap-2 text-lg font-bold text-ink">
          <span class="icon-chip w-8 h-8 bg-emerald-50 text-emerald-600"><AppIcon name="shopping-cart" :size="17" /></span>
          중고장터
        </h1>
        <div class="flex items-center gap-2">
          ...
          <RouterLink v-if="auth.isLoggedIn" to="/market/write" class="btn-primary px-3 py-2 rounded-lg text-xs"><AppIcon name="edit" :size="13" />등록</RouterLink>
        </div>
      </div>
    ...
    <div class="hidden lg:flex items-center justify-between mb-4 flex-wrap gap-2">
      ...
      <div class="flex items-center gap-2 flex-wrap">
        ...
        <RouterLink v-if="auth.isLoggedIn" to="/market/write" class="btn-primary px-3 py-1.5 text-xs"><AppIcon name="edit" :size="13" />등록</RouterLink>
      </div>
    </div>
    ...
    <div class="mb-2">
      ...
      <template v-else>
        ...
        <span v-if="!activeCat" class="text-xs text-ink-muted ml-2">모든 중고 물품을 볼 수 있습니다</span>
      </template>
    </div>
    ...
    <div v-else-if="!items.length" class="py-16 text-center">
      <div class="icon-chip w-14 h-14 bg-gray-100 text-gray-300 mx-auto mb-3"><AppIcon name="shopping-cart" :size="28" :stroke-width="1.5" /></div>
    ...
    <div v-if="activeItem">
      <div class="card overflow-hidden">
        <div class="px-5 py-4">
          <div class="flex items-center gap-2 mb-2">
            <span :class="{'badge-green':activeItem.status==='active','badge-primary':activeItem.status==='reserved','badge-gray':activeItem.status==='sold'}">{{ {active:'판매중',reserved:'예약중',sold:'판매완료'}[activeItem.status] }}</span>
            <span class="text-xs text-ink-muted">{{ activeItem.condition }}</span>
          </div>
          <div class="flex items-center gap-2">
          ...
          <div class="text-xs text-ink-muted mt-1">{{ activeItem.city }}, {{ activeItem.state }} · {{ activeItem.view_count }}회</div>
        </div>
      ...
      <div v-if="auth.user?.id === activeItem.user_id" class="flex gap-2 mt-3 justify-end">
        <RouterLink :to="`/market/write?edit=${activeItem.id}`" class="text-xs text-amber-600 hover:text-amber-700 inline-flex items-center gap-1 transition-colors"><AppIcon name="edit" :size="12" />수정</RouterLink>
        <button @click="deleteItem('market')" class="text-xs text-red-400 hover:text-red-600 inline-flex items-center gap-1 transition-colors"><AppIcon name="trash" :size="12" />삭제</button>
      </div>
    ...
    <div v-else-if="viewMode==='card'" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
      ...
      <div @click="openItem(item)"
        class="card card-hover overflow-hidden cursor-pointer flex"
          ...
          <div>
            ...
            <div class="flex items-center justify-between gap-1 mb-0.5">
              <div class="flex items-center gap-1 flex-wrap min-w-0">
                <span v-if="item.promotion_tier === 'national'" class="text-[11px] bg-red-500 text-white font-bold px-1.5 py-px rounded">🌐 전국구</span>
                <span v-else-if="item.promotion_tier === 'state_plus'" class="text-[11px] bg-blue-500 text-white font-bold px-1.5 py-px rounded">⭐주+</span>
                <span v-else-if="item.promotion_tier === 'sponsored'" class="text-[11px] bg-amber-500 text-white font-bold px-1.5 py-px rounded">📢스폰</span>
                <span v-else-if="isBoosted(item)" class="text-[11px] bg-purple-500 text-white font-bold px-1.5 py-px rounded">🚀 상위노출</span>
                <span v-if="item.is_negotiable" class="badge-green !text-[11px] !px-1.5">가격협의</span>
                <span v-if="item.hold_enabled" class="badge-blue !text-[11px] !px-1.5">홀드가능</span>
              </div>
            ...
            <div class="text-[11px] text-ink-muted mt-0.5">
              <template v-if="!activeCat">{{ item.category || '기타' }} · </template><UserName :userId="item.user?.id" :name="item.user?.name" className="text-[11px] text-ink-muted inline" />
            </div>
            <div class="text-xs text-ink-muted line-clamp-1 mt-1">{{ (item.content || '').slice(0, 60) }}</div>
          </div>
          <!-- 하단: 위치 + 날짜 + 하트 -->
          <div class="text-[11px] text-ink-muted flex items-center gap-1.5 flex-wrap">
            <span v-if="item.city" class="inline-flex items-center gap-0.5"><AppIcon name="map-pin" :size="11" />{{ item.city }}{{ item.state ? ', '+item.state : '' }}</span>
    ...
    <div v-else class="card overflow-hidden divide-y divide-gray-50">
      <template v-for="(item, i) in items" :key="item.id">
      <div @click="openItem(item)"
        class="flex transition-colors cursor-pointer overflow-hidden"
        :class="promoRowClass(item)">
        ...
        <div class="w-28 h-24 flex-shrink-0 bg-gray-100">
          ...
          <div v-else class="w-full h-full flex items-center justify-center bg-gray-100 text-gray-300"><AppIcon name="image" :size="24" :stroke-width="1.5" /></div>
        </div>
        <div class="flex items-center justify-between flex-1 min-w-0 px-4 py-3">
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-1.5 mb-0.5">
              <span v-if="item.promotion_tier === 'national'" class="text-[11px] bg-red-500 text-white font-bold px-1.5 py-0.5 rounded">🌐 전국구</span>
              <span v-else-if="item.promotion_tier === 'state_plus'" class="text-[11px] bg-blue-500 text-white font-bold px-1.5 py-0.5 rounded">⭐ 주+</span>
              <span v-else-if="item.promotion_tier === 'sponsored'" class="text-[11px] bg-amber-500 text-white font-bold px-1.5 py-0.5 rounded">📢 스폰서</span>
              <span v-else-if="isBoosted(item)" class="text-[11px] bg-purple-500 text-white font-bold px-1.5 py-0.5 rounded">🚀 상위노출</span>
            </div>
            ...
            <div class="text-xs text-ink-muted mt-0.5 flex items-center gap-1.5 flex-wrap">
              <UserName v-if="item.user?.id" :userId="item.user?.id" :name="item.user?.name || item.company || item.organizer" className="text-ink-light" /><span v-else-if="item.company || item.organizer">{{ item.company || item.organizer }}</span>
          ...
          <div class="ml-3 flex-shrink-0 text-right">
            ...
            <div v-if="item.salary_min" class="text-amber-600 font-black text-xs">${{ item.salary_min }}~${{ item.salary_max }}/{{ item.salary_type }}</div>
