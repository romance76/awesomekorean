...
<div class="min-h-screen flex flex-col">
  ...
  <div class="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 sticky top-0 z-10">
    <button @click="$router.push('/chat')" class="text-ink-muted hover:text-amber-600 transition-colors"><AppIcon name="arrow-left" :size="20" /></button>
    <h1 class="font-bold text-ink text-sm flex items-center gap-1.5"><AppIcon name="message-circle" :size="15" class="text-amber-600" />{{ roomName }}</h1>
  </div>
  ...
  <div ref="msgArea" class="flex-1 overflow-y-auto px-4 py-3 space-y-3">
    ...
    <div v-for="msg in messages" :key="msg.id"
      ...
      <div class="max-w-[70%]">
        <div v-if="msg.user_id !== auth.user?.id" class="text-[11px] text-ink-muted mb-0.5">{{ msg.user?.name }}</div>
        <div class="px-3 py-2 rounded-2xl text-sm"
          :class="msg.user_id === auth.user?.id ? 'bg-amber-400 text-white rounded-br-md' : 'bg-white shadow-card text-ink rounded-bl-md'">
          {{ msg.content }}
        </div>
    ...
    <div v-if="!loading && !messages.length" class="text-center py-8 text-ink-muted text-sm">메시지가 없습니다. 첫 메시지를 보내보세요!</div>
  </div>
  ...
  <div class="bg-white border-t border-gray-100 px-4 py-3">
    <form @submit.prevent="sendMsg" class="flex gap-2">
      <input v-model="newMsg" type="text" placeholder="메시지 입력..." autofocus
        class="input-soft flex-1 rounded-full px-4 py-2 text-sm" />
      <button type="submit" :disabled="!newMsg.trim()" class="bg-amber-400 text-white font-bold px-5 py-2 rounded-full text-sm shadow-btn hover:bg-amber-500 disabled:opacity-50 transition-colors flex items-center gap-1"><AppIcon name="send" :size="15" />전송</button>
    </form>
  </div>
</div>
</template>
